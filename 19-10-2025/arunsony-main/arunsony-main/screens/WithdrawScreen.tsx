...
export default function WithdrawScreen({ navigation }: WithdrawScreenProps) {
  ...
  const submitWithdrawalRequest = async (withdrawAmount: number, methodName: string) => {
    try {
      ...
      if (response.success) {
        // Store the withdrawal amount before clearing the form
        setLastWithdrawalAmount(withdrawAmount);
        // Show success modal
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <SuccessModal
        ...
        message={`Your withdrawal request of ₹${lastWithdrawalAmount.toLocaleString()} has been submitted successfully. It will be processed within 24-48 hours.`}
