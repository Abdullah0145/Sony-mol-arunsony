...
const ProductsScreen: React.FC = () => {
  ...
  const renderProduct = ({ item }: { item: Product }) => {
    ...
    const getImageUrl = (imageUrl: string | null | undefined, productName: string) => {
      ...
      const getDefaultImage = (name: string) => {
        ...
        return defaultImages[name] || 'https://images.unsplash.com/photo-1560472354-b33ff0c44a43?w=400&h=400&fit=crop&crop=center';
      };

      console.log('🖼️ Processing image for', productName, ':', imageUrl);

      if (!imageUrl || imageUrl.trim() === '') {
        const defaultUrl = getDefaultImage(productName);
        console.log('📷 Using default image for', productName, ':', defaultUrl);
        return defaultUrl;
      }
      
      if (imageUrl.startsWith('http')) {
        console.log('🌐 Using external URL for', productName, ':', imageUrl);
        return imageUrl;
      }
      
      // Handle both old format (/uploads/products/...) and new format (/api/products/image/...)
      if (imageUrl.startsWith('/uploads/products/')) {
        const filename = imageUrl.split('/').pop();
        const newUrl = `https://asmlmbackend-production.up.railway.app/api/products/image/${filename}`;
        console.log('🔄 Converting old format URL for', productName, ':', imageUrl, '->', newUrl);
        return newUrl;
      }
      
      const finalUrl = `https://asmlmbackend-production.up.railway.app${imageUrl}`;
      console.log('🔗 Using backend URL for', productName, ':', finalUrl);
      return finalUrl;
    };
      ...
      <View style={styles.productCard}>
        ...
        <View style={styles.imageSection}>
          <View style={styles.productImageContainer}>
            <FallbackImage
              source={{ uri: getImageUrl(item.imageUrl, item.name) }}
              fallbackUri={getImageUrl(null, item.name)}
