...
export default function WithdrawalHistoryScreen({ navigation }: WithdrawalHistoryScreenProps) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status.toLowerCase()) {
      case 'success':
      case 'completed':
        return <AntDesign name="checkcircle" size={20} color="#00C851" />;
      case 'failed':
        return <AntDesign name="closecircle" size={20} color="#FF4444" />;
      case 'pending':
      default:
        return <AntDesign name="clockcircle" size={20} color="#FFA500" />;
    }
  };
    ...
    <View key={withdrawal.id} style={styles.withdrawalItem}>
      <View style={styles.withdrawalHeader}>
        ...
        <View style={styles.statusContainer}>
          ...
          <Text style={[styles.status, { color: withdrawal.statusColor }]}>
