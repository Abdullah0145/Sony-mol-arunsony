  ...
  const createPaymentOrder = async (testMode: boolean = false) => {
    try {
      if (testMode) {
        ...
        const response = await apiService.createActivationPaymentOrder(user?.userId || 0);
  ...
  const processTestPayment = async () => {
    ...
    try {
      ...
      if (order && order.requiresPayment && order.activationStatus === 'PENDING_PAYMENT') {
        ...
        console.log('Real Razorpay payment result for ₹1:', paymentResult);
        ...
        if (completePaymentResponse.ok) {
          ...
          console.log('Real payment completion result:', paymentResult);
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      if (order && order.requiresPayment && order.activationStatus === 'PENDING_PAYMENT') {
        ...
        const paymentOrder = await createPaymentOrder(false);
      ...
      <View style={styles.footer}>
        ...
        >
          {loading || processingPayment ? (
            ...
            <>
              <AntDesign name="experiment" size={20} color="#000000" />
              <Text style={styles.payButtonText}>
                Real Payment - Pay ₹1
              </Text>
            </>
